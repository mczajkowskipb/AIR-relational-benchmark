cat("=== AIR benchmark: ranktreeEnsemble rforest smoke test on real folds ===\n")
...
method_id <- "rrf_ranktreeensemble"
  ...
  for (repeat_id in repeat_ids) {
    for (fold_id in fold_ids) {
      ...
      prep <- fit_preprocessing(
        split$x_train,
        feature_filter = "mad",
        n_features = min(50, ncol(split$x_train)),
        impute = TRUE,
        scale = FALSE
      )
...
cat("\n=== ranktreeEnsemble rforest smoke summary ===\n")
print(summary_agg)
...
cat("=== ranktreeEnsemble rforest smoke test finished ===\n")
