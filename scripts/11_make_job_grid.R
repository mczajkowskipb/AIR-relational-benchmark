cat("=== AIR benchmark: make job grid ===\n")
...
for (dataset_id in fold_summary$dataset_id) {
  ...
  for (method_id in methods) {
    for (i in seq_len(nrow(combos))) {
      ...
      n_features <- if (method_id == "tspdt_bigtsp") {
        100
...
out_csv <- "results/job_grid/cv_10x10_job_grid.csv"
out_cmd <- "results/job_grid/cv_10x10_commands.txt"
