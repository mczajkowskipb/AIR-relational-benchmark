cat("=== AIR benchmark: switchBox smoke test on real folds ===\n")
...
method_configs <- list(
  switchbox_tsp = list(method_id = "switchbox_tsp", krange = c(1)),
  switchbox_ktsp = list(method_id = "switchbox_ktsp", krange = c(3, 5, 7))
)
...
for (method_id in names(method_configs)) {
  cat("\n\n=== Method:", method_id, "===\n")
    ...
    for (repeat_id in repeat_ids) {
      for (fold_id in fold_ids) {
        ...
        prep <- fit_preprocessing(
          split$x_train,
          feature_filter = "mad",
          n_features = min(200, ncol(split$x_train)),
          impute = TRUE,
          scale = FALSE
        )
        ...
        res <- if (method_id == "switchbox_tsp") {
          fit_predict_internal_tsp(
            x_train = x_train_p,
...
cat("=== switchBox smoke test finished ===\n")
